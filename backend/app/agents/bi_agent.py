...
3. 覆盖 7 种图表类型：bar, line, pie, funnel, gauge, table, ranking
...
### 2. 图表配置
为数据生成尽可能多的图表配置（**至少6-10个**，如果数据维度丰富可以更多），覆盖以下7种图表类型：
...
| 漏斗图 | funnel | 阶段转化分析 | GROUP BY 阶段字段, COUNT(*) |
| 仪表盘 | gauge | 目标达成率/KPI指标 | 单一汇总值 + 目标值 |
...
{{
  "chart_type": "bar|line|pie|funnel|gauge|table|ranking",
  "category": "分类名称",
  "title": "图表标题",
  "description": "图表描述",
...
2. SQL 必须使用反引号包裹列名和表名（MySQL 语法，支持中文列名）
3. SQL 不要包含 WHERE 条件，筛选条件由系统动态追加
4. chart_type 必须是以下之一: bar, line, pie, funnel, gauge, table, ranking
...
class BIClassificationAgent(BaseAgent):
    ...
    VALID_CHART_TYPES = {"bar", "line", "pie", "funnel", "gauge", "table", "ranking"}
    ...
    def _validate_single_chart(
        ...
        if sql and not sql.strip().upper().startswith("SELECT"):
            raise ValueError("图表 SQL 必须是 SELECT")
