          ...
          <template v-else>
            <div class="understand-section">
              <div class="card understand-card">
                ...
                <div
                  v-else-if="understandingContent || regeneratingUnderstanding || verificationStatus === 'verifying'"
                  class="understand-markdown"
                >
                  <div v-if="!understandingContent" class="generating-hint">
                    <div class="generating-hint__dots">
                      <span /><span /><span />
                    </div>
                    <p class="generating-hint__text">AI 正在生成理解内容，请稍候…</p>
                    <button
                      v-if="!isActiveUnderstandingStreamConnected"
                      type="button"
                      class="btn-secondary btn-sm"
                      @click="_subscribeUnderstandingStream(activeTable)"
                    >
                      重新连接生成
                    </button>
                  </div>
                  <span v-html="renderedUnderstanding" />
                  <span v-if="regeneratingUnderstanding" class="typewriter-cursor" />
                </div>
...
const activeUnderstandingStageIndex = computed(() => {
  ...
  if (verificationStatus.value === 'generating' || loadingUnderstanding.value || regeneratingUnderstanding.value) return 1
  return 0
})
...
const showUnderstandingStageProgress = computed(() => {
  return loadingUnderstanding.value || regeneratingUnderstanding.value || verificationStatus.value === 'verifying' || verificationStatus.value === 'generating'
})
...
const activeRelationsStageIndex = computed(() => {
  ...
  if (relationsVerificationStatus.value === 'generating' || loadingRelations.value || regeneratingRelations.value) return 1
  // 如果空间下有文件正在生成推荐问题，显示第5步
  const hasGeneratingRec = tables.value.some((f: any) => f.recommended_questions_status === 'generating')
...
const showRelationsStageProgress = computed(() => {
  return loadingRelations.value || regeneratingRelations.value || relationsVerificationStatus.value === 'verifying' || relationsVerificationStatus.value === 'generating'
})
